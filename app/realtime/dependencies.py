...
async def get_ws_user(user_id: Annotated[str | None, Query(ge=1)] = None) -> str:
    resolved = (user_id or "").strip()
    if not resolved or len(resolved) > MAX_ID_LENGTH:
        raise WebSocketException(code=CLOSE_INVALID_USER, reason="user_id query parameter is required")
